...
export const ProtocolDocumentConverter = () => {
  ...
  const parseProtocolFromText = (text: string): TreatmentProtocol => {
    ...
    let location = "Non spécifié"; // Patterns de détection (inchangés)
    ...
    lines.forEach((line, index) => {
      // ... (Détection PTV et Prescription inchangée) ...
      ...
      if (constraintMatch) {
        ...
        if (type.startsWith("D")) {
          ...
          constraintType = "Vx";
        } // CORRECTION ICI: Ajouter 'cc' comme unité de volume possible (en plus de %)
        const valueMatch = line.match(/[<>=]\s*(\d+(?:\.\d+)?)\s*(Gy|%|cc)/i);
        if (valueMatch) {
          const value = parseFloat(valueMatch[1]);
          const unit = valueMatch[2]; // Extraire le nom de l'organe (avant la contrainte)
          const organMatch = line.match(/^([A-Za-zéèêàâôûçÉÈÊÀÂÔÛÇ\s]+)\s*[:-]/);
          const organName = organMatch ? organMatch[1].trim() : "Organe inconnu"; // Extraire la valeur cible pour Vx et Dx
          let target: number | undefined;
          if (constraintType === "Vx") {
            const vxMatch = type.match(/V(\d+)/);
            if (vxMatch) target = parseFloat(vxMatch[1]); // C'est la dose cible en Gy
          } else if (constraintType === "Dx") {
            const dxMatch = type.match(/D(\d+)/);
            if (dxMatch) target = parseFloat(dxMatch[1]); // C'est le volume cible en % ou cc
          }
          constraints.push({
            organName,
            constraintType,
            value,
            unit: unit.toLowerCase(), // Normaliser l'unité en minuscules ('cc' au lieu de 'Cc' ou 'CC')
            target,
            priority: "mandatory",
            description: line,
          });
              ...
              <div className="space-y-2">
                {protocol.prescriptions.map((presc, idx) => (
                  <div key={idx} className="flex items-center gap-2 text-sm p-2 bg-muted rounded">
                    ...
                    <span>
                      {presc.totalDose} Gy / {presc.numberOfFractions} fx ({presc.dosePerFraction.toFixed(2)} Gy/fx)
                    </span>
                  </div>
                ))}
              </div>
      ...
      <Card className="bg-muted/50">
        ...
        <CardContent className="text-sm space-y-2">
          ...
          <ul className="list-disc list-inside space-y-1 text-muted-foreground">
            <li>
              <strong>Prescriptions:</strong> "PTV_XXX: 45 Gy / 25 fx"
            </li>
            <li>
              <strong>Contraintes Dmax:</strong> "Organe - Dmax &lt; 50 Gy"
            </li>
            <li>
              <strong>Contraintes Dmean:</strong> "Organe - Dmean &lt; 20 Gy"
            </li>
            <li>
              <strong>Contraintes Vx:</strong> "Organe - V20Gy &lt; 15%"
            </li>
            <li>
              <strong>Contraintes Dx:</strong> "Organe - D2% &lt; 60 Gy"
            </li>
          </ul>
        </CardContent>
      </Card>
