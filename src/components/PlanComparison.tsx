...
import { calculateDx, calculateVx, calculateHomogeneityIndex, calculateConformityIndex } from '@/utils/planQualityMetrics';
...
export const PlanComparison = ({ plans }: PlanComparisonProps) => {
  ...
  const commonStructures = useMemo(() => {
    ...
    return firstPlanStructures.filter(name =>
      plans.every(plan => plan.structures.some(s => s.name === name))
    );
  }, [plans]);
  ...
  const metrics = useMemo((): MetricComparison[] => {
    ...
    const structures = plans.map(plan => 
      findStructureByName(plan.structures, selectedStructure)
    );
    ...
    const baseMetrics: MetricComparison[] = [
      ...
      },
      {
        ...
        values: structures.map(s => {
          if (!s?.relativeVolume) return null;
          const totalDose = s.relativeVolume.reduce((sum, point, idx) => {
            const volumeFraction = idx === 0 ? 1 : (s.relativeVolume[idx-1].volume - point.volume) / 100;
            return sum + point.dose * volumeFraction;
          }, 0);
          return totalDose;
        }),
        lowerIsBetter: !isPTV,
        tooltip: 'Dose moyenne reçue par la structure'
      }
    ];
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Analyse comparative des métriques dosimétriques entre {plans.length} plans
          </CardDescription>
        </CardHeader>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b bg-muted/50">
                    <th className="text-left p-4 font-semibold sticky left-0 bg-muted/50 z-10">
                      Métrique
                    </th>
                    {plans.map((plan, idx) => (
        ...
        <Card className="border-orange-500/50 bg-orange-500/5">
          <CardContent className="py-4">
            <p className="text-sm text-orange-600 dark:text-orange-400">
              ⚠️ Attention : Certaines structures ne sont pas présentes dans tous les plans et ne peuvent pas être comparées.
              Seules {commonStructures.length} structures communes sont disponibles pour la comparaison.
            </p>
          </CardContent>
        </Card>
      )}
