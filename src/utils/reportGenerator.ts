...
// ========== COMPACT REPORT (Only Summary + PTVs + OARs + Signature) ==========
export function generateCompactHTMLReport(
  ...
  // CSS Styles (same as full report)
  const styles = `
    @import url('https://fonts.googleapis.com/css2?family=Libre+Baskerville:wght@400;700&display=swap');
  ...
  const html = [];
  html.push('<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8">');
  html.push(`<title>Validation Compacte - ${protocolName}</title>`);
  html.push(`<style>${styles}</style></head><body><div class="report-container">`);
  ...
  if (ptvMetrics.length > 0) {
    html.push('<div class="section">');
    html.push('<div class="section-header">ÉVALUATION DES VOLUMES CIBLES (PTVs)</div>');
    html.push(
      '<p style="font-size:10pt;margin-bottom:10px">Évaluation de la couverture et de l\'homogénéité de la dose dans les volumes tumoraux.</p>',
    );
    html.push('<table class="data-table">');
  ...
  // ========== SECTION: OAR CONSTRAINTS ==========
  if (constraintResults.length > 0) {
    html.push('<div class="section">');
    html.push('<div class="section-header">CONTRAINTES ORGANES À RISQUE (OARs)</div>');
    html.push(
      '<p style="font-size:10pt;margin-bottom:10px">Vérification des contraintes de dose pour protéger les organes sains.</p>',
    );
  ...
  // ========== SECTION: VALIDATION & SIGNATURE ==========
  html.push('<div class="section signature-section" id="section-4">');
  html.push('<div class="section-header">VALIDATION ET SIGNATURE</div>');
  html.push('<table class="info-table">');
  html.push(`<tr><td>Plan validé par</td><td>${doctorName || "___________________________"}</td></tr>`);
  html.push("<tr><td>Date de validation</td><td>____/____/________</td></tr>");
  html.push("</table>");
...
export async function generateCompactPDFReport(
  report: ValidationReport,
  overallStatus?: "PASS" | "FAIL",
  doctorName?: string,
): Promise<Blob> {
  const htmlContent = generateCompactHTMLReport(report, overallStatus, doctorName);
  ...
  try {
    ...
    // Add PDF metadata
    pdf.setProperties({
      title: `Validation Compacte - ${report.protocolName}`,
      subject: "Rapport de Validation du Plan de Traitement - Format Compact",
      author: "DVH Analyzer",
      keywords: "radiothérapie, dosimétrie, validation, DVH, compact",
      creator: "DVH Analyzer v1.0",
    });
      ...
      if (section4Position) {
        ...
        if (section4StartsOnThisPage && section4EndsAfterThisPage) {
          ...
          if (spaceBeforeSection4 > 30) {
            ...
            pdf.setFontSize(10);
            pdf.setFont("helvetica", "normal");
            pdf.text("DVH Analyzer - Rapport Compact", margin.left, margin.top - 5);
            pdf.text(`Page ${pageNumber}`, pageWidth - margin.right - 15, margin.top - 5);
...
export function downloadCompactHTMLReport(
  report: ValidationReport,
  overallStatus?: "PASS" | "FAIL",
  doctorName?: string,
): void {
  const htmlContent = generateCompactHTMLReport(report, overallStatus, doctorName);
  const blob = new Blob([htmlContent], { type: "text/html" });
  const filename = `Validation_Compact_${report.patientId}_${new Date().toISOString().split("T")[0]}.html`;
  downloadFile(blob, filename);
}

export async function downloadCompactPDFReport(
  report: ValidationReport,
  overallStatus?: "PASS" | "FAIL",
  doctorName?: string,
): Promise<void> {
  const blob = await generateCompactPDFReport(report, overallStatus, doctorName);
  const filename = `Validation_Compact_${report.patientId}_${new Date().toISOString().split("T")[0]}.pdf`;
  downloadFile(blob, filename);
}
