...
export function generateUltraCompactHTMLReport(
  ...
  const html = [];
  html.push(`<!DOCTYPE html><html><head><meta charset="UTF-8"><style>${styles}</style></head><body><div class="container">`);
  
  // SECTION 1 : Résumé
  html.push('<div class="section"><div class="header"><h1>Résumé du Plan</h1></div>');
  html.push('<div class="summary-grid">');
  html.push(`<div class="summary-item"><span class="summary-label">Patient ID</span><span>${report.patientId}</span></div>`);
  html.push(`<div class="summary-item"><span class="summary-label">Protocole</span><span>${report.protocolName}</span></div>`);
  html.push(`<div class="summary-item"><span class="summary-label">Statut</span><span class="status-badge" style="background:${overallStatus === 'PASS' ? '#D3F9D8' : '#FFE0E0'};color:${overallStatus === 'PASS' ? '#2D7A3E' : '#C92A2A'}">${overallStatus || report.overallStatus}</span></div>`);
  html.push('</div></div>');
  ...
  if (ptvMetrics.length > 0) {
    ...
    html.push('<table><thead><tr><th>PTV</th><th>D95%</th><th>D98%</th><th>D50%</th><th>D2%</th><th>HI</th><th>CI</th></tr></thead><tbody>');
  ...
  if (constraintResults.length > 0) {
    ...
    html.push('<table class="oartable"><thead><tr><th>Organe</th><th>Contrainte</th><th>Seuil</th><th>Valeur</th><th>Écart</th><th>Priorité</th><th>Statut</th></tr></thead><tbody>');
  ...
  html.push(`<table style="width:100%;margin-bottom:10px;"><tr><td style="width:30%;font-weight:bold;">Plan validé par</td><td style="border-bottom:1px solid #000;">${doctorName || ""}</td></tr>`);
  html.push('<tr><td style="font-weight:bold;">Date</td><td style="border-bottom:1px solid #000;">____/____/________</td></tr></table>');
...
export async function generateTest2PDFReport(
  ...
  try {
    ...
    pdf.setProperties({
      title: `Validation TEST2 - ${report.patientId}`,
    ...
    while (remainingHeight > 0) {
      ...
      pdf.text(`TEST2 - ${report.patientId}`, margin.left, margin.top - 5);
